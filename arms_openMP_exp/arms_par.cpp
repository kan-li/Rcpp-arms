...
// [[Rcpp::export]]
List sampling_par(NumericVector nobs, mat design_matrix, vec y, double myshape, vec temp_beta, vec temp_u, double temp_tau, int n_smp, int n_core ){
...
// cout << it << endl;
    ...
    for(int i=0;i<I;i++){
      start(i)=counter;
      end(i)=counter+nobs(i)-1;
      counter=end(i)+1;
    }
    
    omp_set_num_threads(n_core);
    #pragma omp parallel for shared(temp_u, I, start, end, temp_beta, y, design_matrix, temp_tau, xinit, ninit, xl1, xr1, npoint, dometrop, convex, nsamp,qcent,xcent,ncent,neval) private(err,usamp)
    for(int i=0;i<I;i++){
     ...
     //  cout<<omp_get_num_threads()<<endl;
      ...
      u_data.myy = y.subvec(start(i),end(i));
      u_data.mycovariate = design_matrix.rows(start(i),end(i));
